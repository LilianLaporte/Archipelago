...
namespace error
{
	...
	// City has too much node
	std::string too_much_node();
	
	// A node connect to itself
	std::string link_to_itself();
	
	//No enough node to edit
	std::string not_enough_node();
	
	//No node selected
	std::string not_selected();
}
